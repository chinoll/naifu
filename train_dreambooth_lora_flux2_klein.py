...
def save_model_card(
    ...
    model_description = f"""
# Flux.2 [Klein] Fine-Tuning - {repo_id}

<Gallery />

## Model description

These are {repo_id} fine-tuning weights for {base_model}.

The weights were trained using the [Flux2 diffusers trainer](https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/README_flux2.md).

Quant training? {quant_training}

## Download model

[Download the weights]({repo_id}/tree/main) in the Files & versions tab.
...
image = pipeline('{validation_prompt if validation_prompt else "a photo of a cat"}').images[0]
```
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether to skip the final inference step with loaded weights upon training completion. This will run intermediate validation inference if `validation_prompt` is provided. Specify to reduce memory.",
    )
    parser.add_argument(
...
class FineTuneDataset(Dataset):
    """
    A dataset to prepare the training images with the prompts for fine-tuning the model.
    ...
    def __init__(
        ...
        self.train_data_root = Path(args.train_data_dir)
        if not self.train_data_root.exists():
            raise ValueError("Training data root doesn't exists.")

        # Load images and looking for captions
        accepted_extensions = [".jpg", ".png", ".jpeg", ".webp", ".bmp"]
        for path in list(self.train_data_root.iterdir()):
            if path.suffix.lower() in accepted_extensions:
                self.train_images.append(Image.open(path))
                
                # Try to find corresponding text file
                txt_path = path.with_suffix(".txt")
                if txt_path.exists():
                    with open(txt_path, "r", encoding="utf-8") as f:
                        self.train_prompts.append(f.read().strip())
                else:
                    self.train_prompts.append("")
    ...
    def __getitem__(self, index):
        example = {}
        instance_image, bucket_idx = self.pixel_values[index]
        example["pixel_values"] = instance_image
        example["bucket_idx"] = bucket_idx
        
        caption = self.train_prompts[index]
...
class BucketBatchSampler(BatchSampler):
    def __init__(self, dataset: FineTuneDataset, batch_size: int, drop_last: bool = False):
...
def main(args):
    ...
    # Full fine-tuning: train the entire transformer
    ...
    def load_model_hook(models, input_dir):
        while len(models) > 0:
            ...
            if isinstance(unwrap_model(model), type(unwrap_model(transformer))):
                ...
                loaded_state_dict = Flux2Transformer2DModel.from_pretrained(
    ...
    if accelerator.is_main_process:
        tracker_name = "flux2-klein-finetune"
    ...
    if is_fsdp:
        ...
        if accelerator.is_main_process:
            ...
            transformer.save_pretrained(os.path.join(args.output_dir, "transformer"), state_dict=state_dict)
